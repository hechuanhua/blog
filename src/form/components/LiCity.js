...
export const LiCity=({onclick})=>{
    return( 
        <li onClick={()=>{onclick("CITY")}}><i className="iconfont icon-sheng"></i>省市区</li>
    )
}
export const PreviewCity=({data,actions})=>{
    return <div className={data.active?"previewItem active":"previewItem"} onClick={()=>{actions.clickPreviewLi(data.id)}}>
              ...
              <div className="readOnly">
                <select disabled><option>省</option></select>
                <select disabled><option>市</option></select>
                <select disabled><option>区</option></select>
              </div>
...
export class EditCity extends Component{
    ...
    return <div className="fieldEdit textFieldEdit">
             <div className="title"><i className="fa fa-edit"></i>省市区</div>
